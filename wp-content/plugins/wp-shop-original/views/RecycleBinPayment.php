...
if ($user_delivery == 1){
  ...
  ?>
	<?php  if ($firstPayment == null){
			...
			if( !empty ($thumbnail)){
				echo "<a cost='".$cost_del."' free_delivery='{$free_shiping}' class='img'><img src='".$thumbnail[0]."' /></a><br>";
			}
			echo "<a cost='".$cost_del."' free_delivery='{$free_shiping}' link='".$cost_link."' class='info'>".$del_name."</a>";
			echo "<br>";
