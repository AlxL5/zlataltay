...
function wp_shop_original_cart_info(){
  ...
  if (isset($rows)&&is_array($rows)) {
    foreach ($rows as $key=>$row){
      $result['quantity'] +=$row->quantity*1;
      $result['cost'] +=$row->price*$row->quantity*1;
      ...
      $result['dimentions'][$key] = array($lenght,$width,$height,$row->quantity*1);
      $result['weight'] +=$weight*$row->quantity*1;
